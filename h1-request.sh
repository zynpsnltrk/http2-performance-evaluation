...
for (( j=0; j<${#delayArray[@]}; j=j+1 )) do 
	for (( k=0; k<${#lossArray[@]}; k=k+1 )) do 
		for ((i=0; i < LIMIT ; i=i+1)) do 
			data=$(./nghttp2/src/h2load -n100 -c40 --h1 https://$1/)
			#echo $data
			for d1 in $data
			do
				if [ $bool == true ]
		...
		echo "" >> ./nghttp2/src/h1.txt
                echo "Network delay: ${delayArray[j]}ms , Packet loss: ${lossArray[k]}, Total PDT: $totalPDT"
		echo "Network delay: ${delayArray[j]}ms , Packet loss: ${lossArray[k]}, Total PDT: $totalPDT">> ./nghttp2/src/h1.txt
		totalPDT=0
		echo "" >> ./nghttp2/src/h1.txt
	done 
done
